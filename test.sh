...
test_update_like() {
    print_header "Testing PUT update like"
    read -p "Enter like ID to update: " like_id
    read -p "Enter new user ID: " userId
    read -p "Enter new post ID: " postId
    local update_data=$(cat <<EOF
{
    "userId": $userId,
    "postId": $postId
...
EOF
    ...
    make_request "PUT" "$LIKES_URL/$like_id" "$update_data"
}
...
test_update_follow() {
    ...
    read -p "Enter follow ID to update: " follow_id
    read -p "Enter new follower ID: " followerId
    read -p "Enter new following ID: " followingId
    local update_data=$(cat <<EOF
{
    "followerId": $followerId,
    "followingId": $followingId
...
EOF
    ...
    make_request "PUT" "$FOLLOWS_URL/$follow_id" "$update_data"
}
...
test_update_hashtag() {
    ...
    read -p "Enter new hashtag name: " name
    local update_data=$(cat <<EOF
{
    "name": "$name"
}
...
test_add_hashtag_to_post() {
    print_header "Testing ADD hashtag to post"
    read -p "Enter post ID: " post_id
    read -p "Enter hashtag ID: " hashtag_id
    echo "Request: POST $HASHTAGS_URL/post/$post_id/hashtag/$hashtag_id"
    curl -s -X POST "$HASHTAGS_URL/post/$post_id/hashtag/$hashtag_id" \
...
test_remove_hashtag_from_post() {
    ...
    read -p "Enter post ID: " post_id
    read -p "Enter hashtag ID: " hashtag_id
    echo "Request: DELETE $HASHTAGS_URL/post/$post_id/hashtag/$hashtag_id"
    curl -s -X DELETE "$HASHTAGS_URL/post/$post_id/hashtag/$hashtag_id" \
...
test_feed() {
    ...
    limit=${limit:-10}
    offset=${offset:-0}
    make_request "GET" "$API_URL/feed?userId=$userId&limit=$limit&offset=$offset"
}
...
test_posts_by_hashtag() {
    ...
    limit=${limit:-10}
    offset=${offset:-0}
    make_request "GET" "$API_URL/posts/hashtag/$tag?limit=$limit&offset=$offset"
}
...
test_user_followers() {
    ...
    limit=${limit:-10}
    offset=${offset:-0}
    make_request "GET" "$API_URL/users/$user_id/followers?limit=$limit&offset=$offset"
}
...
test_user_activity() {
    ...
    offset=${offset:-0}
    if [ -n "$type" ]; then
        make_request "GET" "$API_URL/users/$user_id/activity?type=$type&limit=$limit&offset=$offset"
    else
        make_request "GET" "$API_URL/users/$user_id/activity?limit=$limit&offset=$offset"
    fi
}
